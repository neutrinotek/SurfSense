...
from app.connectors.mcpo_connector import MCPOConnector, MCPOResult
from app.db import (
    Chunk,
    Document,
...
class ConnectorService:
    ...
    async def search_mcpo(
        ...
        server_identifier = config.get("MCPO_SERVER", "")
        openapi_url = config.get("MCPO_OPENAPI_URL")

        try:
            base_client = MCPOConnector(
                base_url=base_url,
        ...
        configured_tools = config.get("MCPO_TOOLS") or []
        ...
        if not tools_to_query:
            try:
                tools_to_query = await base_client.list_tools()
            except Exception as exc:
                print(f"Unable to discover MCPO tools from OpenAPI schema: {exc!s}")
                return empty_result, []

        if not tools_to_query:
            return empty_result, []

        aggregated_results: list[tuple[str, MCPOResult]] = []

        for tool_name in tools_to_query:
            try:
                tool_client = base_client.with_tool(tool_name)
            except ValueError as exc:
                print(f"Skipping MCPO tool '{tool_name}': {exc!s}")
                continue

            try:
                tool_results = await tool_client.search(user_query)
            except Exception as exc:
                print(f"Error querying MCPO tool '{tool_name}': {exc!s}")
                continue

            for result in tool_results:
                aggregated_results.append((tool_name, result))
        ...
        async with self.counter_lock:
            for tool_name, result in aggregated_results:
